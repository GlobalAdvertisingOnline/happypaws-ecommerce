...
export function TrackingForm() {
  ...
  const handleEmailLookup = (e: React.FormEvent) => {
    ...
    window.location.href = `mailto:support@happypaws.shop?subject=${subject}&body=${body}`;
  };

  return (
    <>
      {/* SRI: re-generate hash if 17track updates their script:
          curl -sL https://www.17track.net/externalcall.js | openssl dgst -sha384 -binary | openssl base64 -A */}
      <Script
        src="https://www.17track.net/externalcall.js"
        strategy="afterInteractive"
        integrity="sha384-p/aqskQaV9IBEYlGv/tI7MQ7nTu9YMj7YpMfnjne141g2l+7PNO+bvNxgW1yZNSj"
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 bg-white py-3.5 pl-12 pr-4 text-base text-slate-900 placeholder:text-slate-400 focus:border-brand-teal focus:outline-none focus:ring-2 focus:ring-brand-teal/20"
                />
              </div>
              <button
                type="submit"
                disabled={trackingNumber.trim().length < 5}
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-teal px-8 py-3.5 text-base font-semibold text-white shadow-sm transition-all hover:bg-brand-teal-dark hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"
              >
                {loading ? (
            ...
            <div className="mx-auto mt-8 max-w-2xl">
              ...
              <div className="mt-4 text-center">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-teal transition-colors hover:text-brand-teal-dark hover:underline"
                >
