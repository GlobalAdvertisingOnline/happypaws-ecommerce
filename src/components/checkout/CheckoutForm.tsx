...
export function CheckoutForm() {
  ...
  function validatePayment(): boolean {
    ...
    if (!payment.billingSameAsShipping) {
      if (!payment.billingFirstName.trim()) errs.billingFirstName = "First name is required.";
      if (!payment.billingLastName.trim()) errs.billingLastName = "Last name is required.";
      if (!payment.billingAddress1.trim()) errs.billingAddress1 = "Address is required.";
      if (!payment.billingCity.trim()) errs.billingCity = "City is required.";
      if (!payment.billingState) errs.billingState = "State is required.";
      if (!payment.billingZip.trim()) errs.billingZip = "ZIP code is required.";
      else if (!/^\d{5}(-\d{4})?$/.test(payment.billingZip.trim())) errs.billingZip = "Enter a valid ZIP code.";
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!clickRes.ok || !clickData.sessionId) {
        throw new Error(
          clickData.message || "Unable to initialize checkout session. Please try again."
        );
      }
      ...
      if (payment.billingSameAsShipping) {
        // Shipping address as primary (billing = shipping)
        Object.assign(leadBody, {
        ...
        // Billing address as primary fields (CC convention)
        Object.assign(leadBody, {
          ...
          country: payment.billingCountry || "US",
          // Shipping address in ship* fields
      ...
      if (!leadRes.ok) {
        ...
        throw new Error(
          leadData?.message || "Unable to submit shipping information. Please check your details."
        );
      }
      ...
      const shippingCents = subtotal >= SHIPPING.freeThreshold ? 0 : SHIPPING.flatRate;
      const orderRes = await fetch("/api/order", {
        ...
        body: JSON.stringify({
          ...
          cvv: payment.cardCvv,
          billShipSame: payment.billingSameAsShipping ? "1" : "0",
          shippingCents,
      ...
      // Clear sensitive card data from memory before navigation
      setPayment((prev) => ({
        ...prev,
        cardNumber: "",
        cardExpiry: "",
      ...
      // Clear CVV on failed attempt (PCI: never retain CVV after auth attempt)
      setPayment((prev) => ({ ...prev, cardCvv: "" }));
      setOrderError(
  ...
  if (items.length === 0) {
    return (
      <div className="py-12 text-center">
        ...
        <a
          href="/shop"
          className="inline-block rounded-xl bg-brand-teal px-6 py-3 text-sm font-semibold text-white transition-all hover:bg-brand-teal-dark"
        >
          Browse Products
        </a>
      </div>
      ...
      <div className="lg:col-span-2">
        ...
        {step === 3 && (
          <ReviewStep
            ...
            billingName={payment.billingSameAsShipping ? undefined : `${payment.billingFirstName} ${payment.billingLastName}`}
            billingAddress={payment.billingSameAsShipping ? undefined : [
