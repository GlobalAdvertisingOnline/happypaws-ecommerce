...
export function CartSummary({ subtotal, itemCount }: CartSummaryProps) {
  const shippingEstimate = subtotal >= SHIPPING.freeThreshold ? 0 : SHIPPING.flatRate;
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      ...
      <Link
        href="/checkout"
        className="btn-shine flex w-full items-center justify-center rounded-xl bg-brand-teal px-6 py-4 text-base font-semibold text-white transition-all hover:bg-brand-teal-dark hover:shadow-lg"
      >
        Proceed to Checkout
      </Link>
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 border-t border-slate-100 pt-4">
        ...
        <div className="flex flex-col items-center gap-1 text-center">
          <RotateCcw className="h-4 w-4 text-brand-teal" />
